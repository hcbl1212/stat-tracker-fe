      ...
      if(result.status != -1) { //Element exists, do something
        ...
        browser.element('css selector','.login-button',function(result1){
          if(result1.status != -1){
            ...
            browser.element('css selector','.view-stat-link',function(result2){
              if(result1.status != -1){
                browser.click('.view-stat-link')
                .waitForElementVisible('.view-stat',5000)
                .assert.containsText('h1','I am the View Stat template')
                .end()
              }
            });
          } 
